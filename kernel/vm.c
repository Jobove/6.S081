...
char ref_count[(PHYSTOP - KERNBASE) / 4096];
...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while (len > 0)
  {
    ...
    if (copy_on_write_page_fault_handler(pagetable, va0) < 0)
      return -1;
    pa0 = walkaddr(pagetable, va0);
    if (pa0 == 0)
      return -1;
...
copy_on_write_page_fault_handler(pagetable_t pagetable, uint64 va)
{
  ...
  if (mappages(pagetable, va, PGSIZE, (uint64)new_page, (flags & ~PTE_C) | PTE_W) < 0)
    goto err;
